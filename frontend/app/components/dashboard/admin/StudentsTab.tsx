...
export default function StudentsTab({ action }: { action: string | null }) {
	...
	return (
		<div className="flex flex-col gap-6">
			...
			{action === "delete-student" && (
				<div className="bg-red-50 rounded-xl border border-red-200 p-5 max-w-md">
					<form onSubmit={handleDelete} className="flex flex-col gap-3">
						<select className={inputCls} value={deleteId} onChange={e => setDeleteId(e.target.value)} required>
							<option value="">Select student</option>
							{students.map(s => <option key={s.id} value={s.id}>{s.fname} {s.lname}</option>)}
						</select>
						<button className="rounded-lg border border-red-400 px-4 py-2 text-sm font-medium text-red-600 hover:bg-red-100 transition-colors" type="submit">
							Delete
						</button>
					</form>
				</div>
